...
contract("ClaimManager", async (accounts) => {
    ...
    const assertClaim = async (_claimType, _issuer, _signature, _data, _uri) => {
        let claimId = await identity.getClaimId(_issuer, _claimType);
        const [claimType, scheme, issuer, signature, data, uri] = await identity.getClaim(claimId);
    ...
    describe("addClaim", () => {
        it("can recover signature", async () => {
            let label = "test";
            // Claim hash
            let toSign = await identity.claimToSign(identity.address, ClaimType.LABEL, label);
            // Sign using eth_sign
        ...
        it("can add self-claim as manager", async () => {
            ...
            // Claim hash
            let toSign = await identity.claimToSign(identity.address, ClaimType.PROFILE, uri);
            // Sign using CLAIM_SIGNER_KEY
            let signature = web3.eth.sign(addr.claim[0], toSign);

            // Add self-claim as manager
            await assertOkTx(identity.addClaim(ClaimType.PROFILE, Scheme.ECDSA, identity.address, signature, uri, uri, {from: addr.manager[0]}));

            // Check claim
            await assertClaim(ClaimType.PROFILE, identity.address, signature, uri, uri);

            await assertClaims(3, {[ClaimType.LABEL]: 2, [ClaimType.PROFILE]: 1});
        });
        ...
        it("checks signature when adding", async () => {
            ...
            await assertRevert(identity.addClaim(ClaimType.PROFILE, Scheme.ECDSA, identity.address, invalidSignature, uri, uri, {from: addr.manager[0]}));

            // Claim doesn't exist
            let claimId = await identity.getClaimId(identity.address, ClaimType.PROFILE);
        ...
        it("can add self-claim with manager approval", async () => {
            ...
            let r = await assertOkTx(identity.addClaim(ClaimType.PROFILE, Scheme.ECDSA, identity.address, signature, uri, uri, {from: addr.claim[0]}));
        ...
        it("other identity can add a claim", async () => {
            // Claim hash
            let uri = "https://twitter.com/mirceap";
            let toSign = await otherIdentity.claimToSign(identity.address, ClaimType.PROFILE, uri);
            let signature = web3.eth.sign(addr.other, toSign);

            // Deployer calls deployedContract.execute(...), which calls identity.addClaim(...)
            let data = identity.contract.addClaim.getData(ClaimType.PROFILE, Scheme.ECDSA, otherIdentity.address, signature, uri, uri);
            let r = await assertOkTx(otherIdentity.execute(identity.address, 0, data, {from: addr.other}));
            let claimRequestId = findClaimRequestId(r);

            // Claim doesn't exist yet
            await assertClaims(2, {[ClaimType.LABEL]: 2});

            // Approve
            await assertOkTx(identity.approve(claimRequestId, true, {from: addr.manager[0]}));

            // Check claim
            await assertClaim(ClaimType.PROFILE, otherIdentity.address, signature, uri, uri);
            await assertClaims(3, {[ClaimType.LABEL]: 2, [ClaimType.PROFILE]: 1});
        });
    });
    ...
    describe("changeClaim", () => {
        it("can update a self-claim", async () => {
            ...
            let signature = web3.eth.sign(addr.claim[0], toSign);
            // Check claim exists
            await assertClaim(ClaimType.LABEL, identity.address, signature, label, uri);

            // Use same signature to update URI
            await assertOkTx(identity.addClaim(ClaimType.LABEL, Scheme.ECDSA, identity.address, signature, label, newUri, {from: addr.manager[1]}));

            // Check claim was updated
            await assertClaim(ClaimType.LABEL, identity.address, signature, label, newUri);
        ...
        it("checks signature when updating", async () => {
            ...
            await assertRevert(identity.addClaim(ClaimType.LABEL, Scheme.ECDSA, identity.address, invalidSignature, label, newUri, {from: addr.manager[1]}));
        ...
        it("needs approval to update a self-claim", async () => {
            ...
            // Use same signature to update URI
            let r = await assertOkTx(identity.addClaim(ClaimType.LABEL, Scheme.ECDSA, identity.address, signature, label, newUri, {from: addr.claim[0]}));
            let claimRequestId = findClaimRequestId(r);
        ...
        it("other identity can update a claim", async () => {
            let claimId = await identity.getClaimId(otherIdentity.address, ClaimType.LABEL);
            ...
            // Deployer calls deployedContract.execute(...), which calls identity.addClaim(...)
            let data = identity.contract.addClaim.getData(ClaimType.LABEL, Scheme.ECDSA, otherIdentity.address, signature, label, newUri);
            let r = await assertOkTx(otherIdentity.execute(identity.address, 0, data, {from: addr.other}));
            let claimRequestId = findClaimRequestId(r);

            // Claim hasn't been updated yet
            await assertClaim(ClaimType.LABEL, otherIdentity.address, signature, label, uri);

            // Approve
            await assertOkTx(identity.approve(claimRequestId, true, {from: addr.manager[0]}));

            // Check claim
            await assertClaim(ClaimType.LABEL, otherIdentity.address, signature, label, newUri);
            await assertClaims(2, {[ClaimType.LABEL]: 2});
        });
    })
    ...
    describe("removeClaim", async () => {
        it("can remove a claim", async () => {
            // First claim
            let claimId = await identity.getClaimId(identity.address, ClaimType.LABEL);
            ...
            await assertClaims(1, {[ClaimType.LABEL]: 1});
        });
    ...
    describe("refreshClaim", () => {
        it ("keeps claims that are valid", async () => {
            let selfId = await identity.getClaimId(identity.address, ClaimType.LABEL);
            let otherId = await identity.getClaimId(otherIdentity.address, ClaimType.LABEL);
        ...
        it ("removes claims that are invalid", async () => {
            ...
            await assertClaims(0, {[ClaimType.LABEL]: 0});
